...
/*Selecione os funcionários em que o cpf do supervisor possui o valor nulo;*/
select  pnome, unome, cpf from funcionario
where funcionario.cpf is null;

/*Selecione os funcionários que não são do sexo feminino;*/
select  pnome, unome, cpf, sexo from funcionario
where funcionario.sexo != 'F';

/*Selecione os funcionários em que o cpf do supervisor não é nulo ou ganham mais que 35000*/
select pnome, unome, cpf, salario from funcionario
where funcionario.cpf is not null 
and funcionario.salario <= 35000;

/*Selecione os funcionários homens em que a data de nascimento é maior que 01/01/1962.*/
select pnome, unome, cpf, salario, datanasc 
from funcionario
where funcionario.datanasc > "1962/01/01";

/*Selecione os funcionários que tem o sobrenome ‘Silva’ ou que o nome seja ‘Jorge’.*/
select pnome, unome 
from funcionario
where funcionario.unome like 'Silva%' or funcionario.pnome like 'Jorge%';

/*Selecione os funcionários que não pertençam a nenhum departamento.*/
select *
from funcionario
...
select * 
from departamento
where departamento.cpf_gerente is not null;

/*Selecione o departamento de nome Matriz;*/
select * 
from departamento
where departamento.dnome = "Matriz";

/*Selecione o departamento que não possui data inicio gerente cadastrada.*/
select *
from departamento
where departamento.data_inicio_gerente is null;

/*Selecione as dependentes que são esposas;*/
select *
from dependentes
where dependente.sexo = 'F' and dependente.parentesco = "Esposa";

/*Selecione as dependentes que são esposas e filhas;*/
select *
from dependente
where dependente.parentesco = "Esposa" and dependente.parentesco = "Filha";

/*Selecione os dependentes em que o cpf do funcionário é 12345678966;*/
select * 
from dependente
where dependente.fcpf = 12345678966;

/*Selecione os dependentes que são homens ou filhos.*/
select * 
from dependente
where dependente.sexo = 'M' or dependente.parentesco = "Filho";
...
/*Selecione os projetos localizados em São Paulo e que o departamento 
seja 5 ou os projetos localizados em São Paulo e que o departamento seja 1*/
select *
from projeto, departamento
where (projeto.projlocal = "São Paulo" and projeto.dnum = 5) 
or (projeto.projlocal = "São Paulo" and projeto.dnum = 1);
